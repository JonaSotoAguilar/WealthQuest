...
public class PlayerNetMovement : MonoBehaviour
{
    [SerializeField] private float speedMovement;
    [SerializeField] private PlayerNetAnimator animator;
    ...
    private IEnumerator Move(int steps, int currPosition)
    {
        ...
        for (int i = 0; i < steps; i++)
        {
            ...
            currPosition++;
            if (currPosition >= GameNetManager.Squares.Length) currPosition = 0;

            // Configurar la casilla de destino y su posición central
            Transform squareTransform = GameNetManager.Squares[currPosition].transform;
    ...
    {
        Transform squareTransform = GameNetManager.Squares[position].transform;
        Square currentSquare = GameNetManager.Squares[position];
        ...
        int nextPosition = (position + 1) % GameNetManager.Squares.Length;
        Vector3 nextSquarePosition = GameNetManager.Squares[nextPosition].transform.position;
    ...
    public void AddCornerPosition(int position) => GameNetManager.Squares[position].AddPlayer(this, position);
}
