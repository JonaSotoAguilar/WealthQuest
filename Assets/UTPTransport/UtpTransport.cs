	...
	public class UtpTransport : Transport
	{
		...
		private void Awake()
		{
			...
			server = new UtpServer(
				(connectionId) => { string clientAddress = server.GetClientAddress(connectionId); OnServerConnectedWithAddress.Invoke(connectionId, clientAddress); },
		...
		private void SetupDefaultCallbacks()
		{
			...
			{
				OnServerConnectedWithAddress = (connectionId, clientAddress) =>
				{
					UtpLog.Warning($"OnServerConnectedWithAddress called with no handler. Connection ID: {connectionId}, Address: {clientAddress}");
				};
			}
